...
class AutoRes(Screen):
	...
	def setMode(self, mode, set=True):
		...
		if porta == "DVI":
			porta = "HDMI"
		resolutionlabel["restxt"].setText("Videomode: %s %s %s" % (porta, mode, rate))
		if set:
...
class ResolutionLabel(Screen):
	skin = """
		<screen position="80,40" size="250,36" flags="wfNoBorder" >
	...
	if config.skin.xres.value == 1920:
		skin = """
			<screen position="80,40" size="300,50" flags="wfNoBorder" >
				<widget name="content" position="0,0" size="300,25" font="Regular;23" />
				<widget name="restxt" position="0,25" size="300,25" font="Regular;23" />
			</screen>"""
